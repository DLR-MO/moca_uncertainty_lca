...
def run_uncertainty_lca():
    ...
    mc_lca = ulca.MonteCarloLCA(demand, lcia_methods=method, run_parallel=False)
    ...
    print(
        f"Time elapsed for uncertainty_lca: {duration:.2f} seconds = {duration // 60:.0f}:{int(duration % 60):02} minutes = {dur_timedelta}"
    )
...
def run_brightway():
    ...
    print(
        f"Time elapsed for brightway2: {duration:.2f} seconds = {duration // 60:.0f}:{int(duration % 60):02} minutes = {dur_timedelta}"
    )
