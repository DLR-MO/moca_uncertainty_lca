...
def plot_convergence(parameter):
    ...
    # Save the figure
    plt.savefig(set_filename(f'convergence_lyfe.png', folder='outputs'), dpi=900)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(set_filename(f'results_lyfe.png', folder='outputs'), dpi=900)
...
# only take the first 50 files
files = files[:10]

# Go through every "_eco.xlsx" file in the output folder
for file in files:
    file_path = os.path.join(output_folder_path, file)

    # Read the LCIA method indicated in cell B11
    lcia_method = pd.read_excel(file_path, usecols="B", skiprows=10, nrows=0).columns[0].replace("'", "")

    # Ensure the LCIA method is supported
    ...
    # Read the relevant data for EF v3.0 no LT
    df = pd.read_excel(file_path, usecols="B:I", skiprows=30, nrows=16)
    input_data.append(df)
    
    cc_life_list.append(pd.read_excel(file_path, usecols="I", skiprows=37, nrows=0).columns[0])
...
cc_life_list = values[(values >= 0) & (values <= (mean + 6 * std))]
